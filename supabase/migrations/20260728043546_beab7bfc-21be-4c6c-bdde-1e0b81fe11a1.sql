...
DECLARE
  ...
  v_sequence integer;
  v_number text;
  v_required text[] := ARRAY['payment_verified','certificate_released','instrument_delivered'];
...
BEGIN
  ...
  IF v_missing_count > 0 THEN
    RAISE EXCEPTION 'Checklist final kalibrasi belum lengkap';
  END IF;
  ...
  LOOP
    IF v_item.certificate_number IS NULL THEN
      ...
      WHERE id = v_item.id
      RETURNING id, certificate_number, certificate_issued_at
      INTO item_id, certificate_number, certificate_issued_at;
...
GRANT EXECUTE ON FUNCTION public.issue_calibration_certificates(uuid) TO authenticated;