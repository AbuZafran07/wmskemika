...
BEGIN
  ...
  -- Find next sequence for the day (based on any existing spk_number pattern SPK/YYYYMMDD.NN)
  SELECT COALESCE(MAX(
    NULLIF(regexp_replace(spk_number, '^SPK/' || v_date_prefix || '\.', ''), '')::int
  ), 0) + 1
    INTO v_seq
  FROM public.sales_order_headers
  WHERE spk_number LIKE 'SPK/' || v_date_prefix || '.%';
