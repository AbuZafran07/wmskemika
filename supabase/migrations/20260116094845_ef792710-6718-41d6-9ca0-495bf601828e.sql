-- Fix Plan Order RPC functions: ref_id columns are UUID type, not text

CREATE OR REPLACE FUNCTION public.plan_order_create(
  header_data jsonb,
  items_data jsonb,
  attachment_meta jsonb DEFAULT NULL::jsonb
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Insert items (generated columns will be computed by DB)
  ...
  -- Insert attachment if provided (ref_id is UUID, no text cast)
  ...
  -- Insert audit log (ref_id is UUID, no text cast)
...
BEGIN
  ...
  -- Insert new items (generated columns computed by DB)
  FOR item IN SELECT * FROM jsonb_array_elements(items_data)
  LOOP
    INSERT INTO plan_order_items (
      plan_order_id,
