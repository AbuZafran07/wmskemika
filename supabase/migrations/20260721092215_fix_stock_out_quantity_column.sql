-- Fix: re-deploy stock_out functions & available_stock view with the correct
-- column name (qty_out) in case the live database still has a stale/drifted
-- definition referencing a non-existent "quantity" column
-- (error seen in app: "column soi.quantity does not exist").
-- CREATE OR REPLACE is idempotent: safe to run even if the live DB is already correct.
