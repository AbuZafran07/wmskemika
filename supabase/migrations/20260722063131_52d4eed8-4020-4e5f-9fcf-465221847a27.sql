...
ALTER TABLE public.sales_order_headers
  ...
  CHECK (
    calibration_status IS NULL OR calibration_status IN (
      'draft',
      'pending_receipt',
      'received',
      'spk_issued',
...
WITH pending AS (
  SELECT h.id,
         'SPK/' || to_char(COALESCE(c.checked_at, h.updated_at, h.created_at, now()), 'YYYYMMDD') || '.' ||
         lpad(
           row_number() OVER (
             PARTITION BY to_char(COALESCE(c.checked_at, h.updated_at, h.created_at, now()), 'YYYYMMDD')
             ORDER BY COALESCE(c.checked_at, h.updated_at, h.created_at, now()), h.id
           )::text,
           2,
           '0'
         ) AS generated_spk_number,
         COALESCE(c.checked_at, h.updated_at, h.created_at, now()) AS generated_spk_issued_at
  FROM public.sales_order_headers h
  JOIN public.calibration_tracker_checklists c
    ON c.sales_order_id = h.id
   AND c.checklist_key = 'spk_issued'
   AND c.is_checked = true
  WHERE h.order_type = 'calibration'
    AND COALESCE(h.is_deleted, false) = false
    AND (h.spk_number IS NULL OR length(trim(h.spk_number)) = 0)
)
UPDATE public.sales_order_headers h
   SET spk_number = pending.generated_spk_number,
       spk_issued_at = pending.generated_spk_issued_at,
       calibration_status = 'spk_issued'
  FROM pending
 WHERE h.id = pending.id;