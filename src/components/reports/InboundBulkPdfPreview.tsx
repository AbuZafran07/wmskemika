...
interface InboundBulkPdfPreviewProps {
  open: boolean;
...
export function InboundBulkPdfPreview({ open, onOpenChange, records, filterDescription }: InboundBulkPdfPreviewProps) {
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      pdf.save(`inbound-report-${new Date().toISOString().split('T')[0]}.pdf`);
      ...
      <DialogContent className="max-w-5xl max-h-[95vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {language === 'en' ? 'Print Inbound Report (Bulk)' : 'Cetak Laporan Penerimaan (Keseluruhan)'}
          </DialogTitle>
        </DialogHeader>
        ...
        <div className="border rounded-lg p-6 bg-white text-black" ref={contentRef}>
          ...
          <div className="flex items-start justify-between mb-4 pb-3 border-b-2 border-gray-800">
            <div className="flex items-center gap-4">
              <img src="/logo-kemika.png" alt="Kemika Logo" className="h-12 object-contain" crossOrigin="anonymous" />
              <div>
            ...
            <div className="text-right">
              <h2 className="text-base font-bold text-gray-900">LAPORAN PENERIMAAN</h2>
              <p className="text-xs text-gray-600">Inbound Report</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-3 gap-3 mb-4">
            ...
            <div className="border border-gray-300 rounded p-2 text-center">
              <p className="text-xs text-gray-500">Total Qty Diterima</p>
              <p className="text-lg font-bold text-green-600">{totalQty.toLocaleString()}</p>
            </div>
          </div>
          ...
          <table className="w-full border-collapse text-xs mb-4">
            <thead>
              <tr className="bg-gray-800 text-white">
                ...
                <th className="border border-gray-300 px-2 py-1.5 text-left">No. Stock In</th>
                <th className="border border-gray-300 px-2 py-1.5 text-left">Tanggal</th>
                <th className="border border-gray-300 px-2 py-1.5 text-left">Plan Order</th>
                <th className="border border-gray-300 px-2 py-1.5 text-left">Supplier</th>
                  ...
                  <tr key={`${record.id}-${item.id}`} className={rIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                    {iIdx === 0 && (
                      <>
                        ...
                        <td className="border border-gray-300 px-2 py-1 font-medium" rowSpan={record.items.length}>{record.stock_in_number}</td>
                        <td className="border border-gray-300 px-2 py-1" rowSpan={record.items.length}>{formatDate(record.received_date)}</td>
                        <td className="border border-gray-300 px-2 py-1" rowSpan={record.items.length}>{record.plan_order?.plan_number || '-'}</td>
                        <td className="border border-gray-300 px-2 py-1" rowSpan={record.items.length}>{record.plan_order?.supplier?.name || '-'}</td>
                      </>
                    )}
                    ...
                    <td className="border border-gray-300 px-2 py-1 text-center font-semibold">{item.qty_received}</td>
                    <td className="border border-gray-300 px-2 py-1">{item.batch_no || '-'}</td>
                    <td className="border border-gray-300 px-2 py-1">{item.expired_date ? formatDate(item.expired_date) : '-'}</td>
                  </tr>
                ))
              )}
            </tbody>
            <tfoot>
              <tr className="bg-gray-100 font-bold">
                <td colSpan={7} className="border border-gray-300 px-2 py-1.5 text-right">Total Qty Diterima:</td>
