  ...
  if (isMinimized) {
    return (
      <Button
        ...
        size="icon"
      >
        <img src={ktalkIcon} alt="K'talk" className="h-12 w-12 object-contain" />
        {unreadCount > 0 && (
    ...
    <Card className={`fixed bottom-4 right-4 shadow-xl z-50 transition-all duration-300 ${isExpanded ? 'w-[500px] h-[600px]' : 'w-[350px] h-[450px]'}`}>
          ...
          {selectedUser ? (
            <span className="flex items-center gap-2">
              <Avatar className="h-6 w-6">
                ...
                <AvatarFallback className="text-xs">{getInitials(selectedUser.name || selectedUser.email)}</AvatarFallback>
              </Avatar>
      ...
      <CardContent className="flex flex-col h-[calc(100%-60px)] p-3 pt-0">
        ...
        <div className="flex gap-1 mb-2 overflow-x-auto pb-1">
          <Button variant={selectedUser === null ? 'default' : 'outline'} size="sm" className="shrink-0 text-xs h-7" onClick={() => setSelectedUser(null)}>Global</Button>
          {onlineUsers.filter((u) => u.id !== user?.id).map((onlineUser) => (
            <Button key={onlineUser.id} variant={selectedUser?.id === onlineUser.id ? 'default' : 'outline'} size="sm" className="shrink-0 text-xs h-7" onClick={() => setSelectedUser(onlineUser)}>
          ...
          <div className="space-y-3">
                  ...
                  <div key={msg.id} className={`flex gap-2 ${isOwnMessage ? 'flex-row-reverse' : ''} group`}>
                    {!isOwnMessage && (
                      <Avatar className="h-7 w-7 shrink-0">
                        ...
                        <AvatarFallback className="text-xs">{getInitials(msg.sender?.full_name || msg.sender?.email || 'U')}</AvatarFallback>
                      </Avatar>
                    )}
                    <div className={`max-w-[75%] ${isOwnMessage ? 'text-right' : ''}`}>
                      {!isOwnMessage && (
                        <p className="text-xs text-muted-foreground mb-0.5">{msg.sender?.full_name || msg.sender?.email?.split('@')[0] || 'User'}</p>
                      )}
                      
                      {/* Reply preview */}
                      {msg.reply_to && (
                        <div className={`text-xs bg-muted/50 rounded px-2 py-1 mb-1 border-l-2 border-primary ${isOwnMessage ? 'ml-auto' : ''}`}>
                          <p className="text-muted-foreground font-medium">{msg.reply_to.sender?.full_name || 'User'}</p>
                          <p className="truncate text-muted-foreground">{msg.reply_to.message.slice(0, 50)}{msg.reply_to.message.length > 50 ? '...' : ''}</p>
                        </div>
                      )}
                      ...
                      <div className="relative">
                        <div className={`rounded-lg px-3 py-2 text-sm ${isOwnMessage ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}>
                          {/* File attachment */}
                          {msg.file_url && (
                            <div className={`mb-2 ${msg.message && msg.message !== `📎 ${msg.file_name}` ? 'pb-2 border-b border-current/20' : ''}`}>
                            ...
                            <PopoverContent side="top" className="w-auto p-2">
                              <div className="flex gap-1">
                                {QUICK_REACTIONS.map((emoji) => (
                                  <button key={emoji} className="text-lg hover:scale-125 transition-transform" onClick={() => toggleReaction(msg.id, emoji)}>
                                    {emoji}
                                  </button>
                                ))}
                              </div>
            ...
            {activeTypingUsers.length > 0 && (
              <div className="flex items-center gap-2 text-muted-foreground">
                ...
                <div className="flex items-center gap-1">
                  <span className="text-xs">{activeTypingUsers.length === 1 ? `${activeTypingUsers[0].name.split(' ')[0]} sedang mengetik` : `${activeTypingUsers.length} orang sedang mengetik`}</span>
        ...
        {replyingTo && (
          <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-t-lg border-l-2 border-primary mt-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground truncate">{replyingTo.message.slice(0, 50)}{replyingTo.message.length > 50 ? '...' : ''}</p>
            </div>
        ...
        {selectedFile && (
          <div className="flex items-center gap-2 p-2 bg-muted/50 rounded-t-lg mt-2">
            ...
            <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => { setSelectedFile(null); if (fileInputRef.current) fileInputRef.current.value = ''; }}>
              <X className="h-3 w-3" />
            </Button>
          </div>
        )}
        ...
        {showMentionList && filteredMentionUsers.length > 0 && (
          <div className="absolute bottom-20 left-3 right-3 bg-popover border rounded-lg shadow-lg max-h-32 overflow-y-auto z-10">
            {filteredMentionUsers.slice(0, 5).map((mentionUser) => (
              <button key={mentionUser.id} className="w-full flex items-center gap-2 p-2 hover:bg-muted transition-colors text-left" onClick={() => insertMention(mentionUser)}>
                <Avatar className="h-6 w-6">
        ...
        <div className={`flex gap-2 ${replyingTo || selectedFile ? '' : 'mt-2'}`}>
          ...
          <Popover open={showEmojiPicker} onOpenChange={setShowEmojiPicker}>
            ...
            <PopoverContent side="top" align="start" className="w-auto p-0 border-0" sideOffset={8}>
              <EmojiPicker onEmojiClick={handleEmojiClick} theme={theme === 'dark' ? Theme.DARK : Theme.LIGHT} width={300} height={350} searchPlaceholder="Cari emoji..." previewConfig={{ showPreview: false }} />
            </PopoverContent>
          </Popover>
          ...
          <Button variant="ghost" size="icon" className="shrink-0" onClick={() => { setNewMessage(prev => prev + '@'); setShowMentionList(true); setMentionStartIndex(newMessage.length); setMentionSearch(''); inputRef.current?.focus(); }}>
