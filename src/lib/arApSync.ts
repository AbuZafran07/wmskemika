...
// API key AR/AP disimpan & dipakai sepenuhnya di sisi server (edge function `arap-sync`).
// Tidak ada API key yang pernah dikirim ke browser.

// Dipertahankan untuk kompatibilitas pemanggil lama (tidak ada cache key di client lagi).
export function clearApiKeyCache() {
  /* no-op: API key tidak lagi disimpan di client */
}
...
async function sendToArAp(
  ...
  try {
    ...
    if (error) {
      console.error(`[AR/AP Sync] ${entity} failed`);
      return { success: false, error: error.message || 'Sync gagal' };
    }

    if (result && (result as any).success === false) {
      return { success: false, error: (result as any).error || 'Sync gagal' };
    }
