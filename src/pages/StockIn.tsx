...
export default function StockIn() {
  ...
  const handleSave = async () => {
    ...
    for (const item of validItems) {
      if (!item.batch_no) {
        toast.error(language === 'en' ? `Please enter batch number for ${item.product_name}` : `Masukkan nomor batch untuk ${item.product_name}`);
        return;
      }
      if (item.qty_received > item.qty_remaining) {
        toast.error(language === 'en' ? `Quantity received cannot exceed remaining for ${item.product_name}` : `Kuantitas diterima tidak boleh melebihi sisa untuk ${item.product_name}`);
        return;
      }
    }
    ...
    try {
      ...
      for (const item of validItems) {
        ...
        await supabase
          .from('plan_order_items')
          .update({ 
            qty_received: (poItem?.qty_received || 0) + item.qty_received,
            qty_remaining: item.qty_remaining - item.qty_received,
          })
          .eq('id', item.plan_order_item_id);
      }
      ...
      console.error(error);
      toast.error(language === 'en' ? 'Failed to save Stock In' : 'Gagal menyimpan Stock In');
    }
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              {language === 'en' ? 'Receive goods from approved Plan Orders' : 'Terima barang dari Plan Order yang disetujui'}
            </p>
          </div>
        </div>
        <Button variant="outline" size="sm" onClick={() => { setPlanOrders([]); setLoadingPlanOrders(true); }}>
        ...
        <CardContent>
          <div className="space-y-2">
            ...
            <Select value={selectedPlanOrderId} onValueChange={setSelectedPlanOrderId}>
              <SelectTrigger>
                <SelectValue placeholder={loadingPlanOrders ? 'Loading...' : (language === 'en' ? '-- Select Plan Order --' : '-- Silakan pilih Plan Order --')} />
              </SelectTrigger>
              <SelectContent>
                {planOrders.map(po => (
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                value={selectedPlanOrder?.plan_number || ''}
                disabled
                className="bg-muted"
              />
            </div>
            <div className="space-y-2">
              ...
              <Input
                value={selectedPlanOrder?.supplier?.name || ''}
                disabled
                className="bg-muted"
              />
            </div>
